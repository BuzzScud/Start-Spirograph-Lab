...
const M = 60000;
/** Bars every traded minute in [from, to): a sine day plus a slow drift, with `level` added and `vol` volume. */
export function synthBars(from, to, { level = 20000, vol = 100, seed = 1 } = {}) {
  ...
  for (let t = from; t < to; t += M) {
    ...
    walk += rnd() * 2;
    const c = level + walk + 20 * Math.sin(2 * Math.PI * t / (240 * M)) + 5 * Math.sin(2 * Math.PI * t / (24 * M));
